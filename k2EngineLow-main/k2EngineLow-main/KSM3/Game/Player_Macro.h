#pragma once
//プレイヤーのカラー用のmap
const std::map<int, const char*> player_color = { // キーと文字列を対応させた連想配列を作成する
...
const char* getPlayer_color(int key) {
	auto it = player_color.find(key); // キーによって対応する文字列を探す
	if (it != player_color.end()) { // 対応する文字列があればそれを返す
		return it->second;
	}

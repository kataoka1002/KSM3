...
void Left_leg_weapons::InitWeapon()
{
	...
	//武器の細かい設定を行う	
	SetWeapon(m_player->p_custom_point[1][2]);
...
void Left_leg_weapons::Update() 
{
	...
	if (m_player->game_state == 0) 
	{
		...
		//攻撃処理
		MakeBullet(m_player->p_custom_point[1][2]);
...
void Left_leg_weapons::SetBulletLocalPosition()
{
	...
	//武器がマシンガンの場合
	if (m_player->p_custom_point[1][2] == 2)
	{
	...
	//武器がギガトンキャノンの場合
	else if (m_player->p_custom_point[1][2] == 4)
	{
	...
	//武器が戦艦砲の場合
	else if (m_player->p_custom_point[1][2] == 6)
	{
